...
public class HotelDao {
	 ...
	 */
	public List<Hotel> getAvailableHotelsOnDates(String startDate, String endDate) throws SQLException {
		PreparedStatement preparedStatement = connection.prepareStatement("SELECT DISTINCT hotel.* from hotel "
				+ "WHERE hotel.id IN " + "(Select room.hotel_id FROM room WHERE room.id NOT IN "
		...
		while (resultSet.next()) {
			hotels.add(new Hotel(resultSet.getInt(ID), resultSet.getString(HOTEL_NAME), resultSet.getInt(CITY_ID)));
		}
		return hotels;
	}
	...
	public int getClientCountForPeriod(int hotel_id, String dateStart, String dateEnd) throws SQLException {
		PreparedStatement preparedStatement = connection.prepareStatement(
				"SELECT COUNT(client_id) FROM room_book WHERE " + "(order_start >= ? AND order_end <= ? AND room_id IN "
	...
	public int getAverageBookTime(int hotel_id, String dateStart, String dateEnd) throws SQLException {
		...
		PreparedStatement preparedStatement = connection
				.prepareStatement("SELECT order_start, order_end FROM room_book WHERE "
		...
		return bookDays.size() > 0 ? bookDays.stream().mapToInt(Integer::intValue).sum() / bookDays.size() : 0;
