...
public class RoomDao {
	...
	public List<Room> getAvailableRoomsOnDatesInHotel(String startDate, String endDate, int hotelId)
		...
		PreparedStatement preparedStatement = connection.prepareStatement(
				"SELECT room.* " + "FROM room " + "WHERE room.id NOT IN (SELECT room_book.room_id FROM room_book "
						+ "WHERE order_start > ? AND (order_end > ? OR order_end > ?))" + "AND room.hotel_id = ?");
		...
		while (resultSet.next()) {
			rooms.add(new Room(resultSet.getInt(ID), resultSet.getInt(ROOM_NUMBER), resultSet.getInt(HOTEL_ID)));
		}
		return rooms;
	}
}
