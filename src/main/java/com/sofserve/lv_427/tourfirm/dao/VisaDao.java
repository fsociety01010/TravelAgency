...
public class VisaDao {
	...
	public int getVisasCountForTheClient(int clientId) throws SQLException {
		PreparedStatement preparedStatement = connection
				.prepareStatement("SELECT COUNT(ID) FROM visa WHERE client_Id = ?");
	...
	public List<Visa> getVisasForTheClient(int clientId) throws SQLException {
		...
		PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM visa WHERE client_Id = ?");
		preparedStatement.setInt(1, clientId);
		ResultSet resultSet = preparedStatement.executeQuery();
		while (resultSet.next()) {
			visas.add(new Visa(resultSet.getInt(ID), resultSet.getString(VISA_NAME), resultSet.getInt(COUNTRY_ID),
